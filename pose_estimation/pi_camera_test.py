...
# Replace VideoCapture with the GStreamer pipeline for Raspberry Pi camera
gstreamer_pipeline = (
    "libcamera-vid -t 0 --inline --width 640 --height 480 --framerate 30 --codec mjpeg | "
    "gst-launch-1.0 fdsrc ! jpegdec ! videoconvert ! appsink"
)
cap = cv2.VideoCapture(gstreamer_pipeline, cv2.CAP_GSTREAMER)

if not cap.isOpened():
    print("Unable to open Raspberry Pi camera")
    exit()
...
while cap.isOpened():
    ...
    if pose_results.pose_landmarks:
        ...
        left_shoulder_coords = tuple(
            np.multiply([left_shoulder.x, left_shoulder.y], [640, 480]).astype(int)
        )
        cv2.putText(
            frame,
            f"{int(left_angle)}°",
